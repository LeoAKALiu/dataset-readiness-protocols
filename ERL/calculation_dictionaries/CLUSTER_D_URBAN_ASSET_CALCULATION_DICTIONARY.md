...
分类、分割和检测的标签单位、标注误差和性能终点不同，不能共用一个 ERL 等级：
...
同一路线连续帧、同一资产多视角和相邻街景不能作为独立 ICU。
...
5. 视频、街景或多视角分组信息；
6. 官方或重建训练/验证划分；
7. 忽略区、遮挡、截断和 crowd 处理规则；
8. 全部裁块、缩放、重编码和标签转换记录；
9. 标注质量审计与裁决结果。

本字典继承总体协议的全部通用判据。C1.4 由数据清单、版本标识和内容哈希直接
裁决，不另造数值指标。当前字典针对像素/图像级识别；若应用范围要求实际尺寸、
...
`urban asset` 等宽泛名称不能在没有操作定义时视为等价。判据：C1.1。
...
同时报告边界 F-score 或物理/像素表面距离。对象或类别先宏平均，不以全部像素
池化替代。判据：C2.1。
...
分别记录城市、区段、批次和相机字段来源。GPS 缺失不必自动失败，但无法恢复路线
...
优先留一城市；城市数不足时依次使用区域、路线或采集批次，但必须降低证据外推
