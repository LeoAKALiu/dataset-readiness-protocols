...
超出范围时输出 `UNRATED_OUT_OF_SCOPE`，不得因缺少配对模态而判低 ERL。
...
该指标描述模态利用，不等于跨域泛化，也不能因融合收益小自动降低 ERL。当前角色：
诊断；判据：C4.1、C4.5。
...
至少输出留一资产、留一地点和留一传感器系统中可执行的层；不允许把同一资产不同
