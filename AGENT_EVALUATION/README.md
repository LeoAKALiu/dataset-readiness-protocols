...
Agent 如何检索和核验证据、确定性程序如何计算指标、规则引擎如何生成等级，
以及如何验证这套评价方法的准确性和重复性。

本目录不修改 DRRL 或 ERL 的构念和等级定义。规范性评分依据仍是：
...
- [`STAGE_0_RUNBOOK.md`](STAGE_0_RUNBOOK.md)：8 家族阶段 0 试运行与执行框架验收。
...
- [`PREREGISTRATION_AND_COMPLETION.md`](PREREGISTRATION_AND_COMPLETION.md)：正式锁、变更控制和完成标志。
...
当前文件是**阶段 0 前的候选执行包**。阶段 0 可以修复歧义、schema、工具适配
和不可计算条目。正式实验只能在阶段 0 通过并生成公开 `protocol_lock` 后启动。
阶段 0 输出不进入正式评分或模型优劣比较。
...
2. 可计算指标由冻结程序生成，等级由确定性规则引擎生成。
3. DRRL 与 ERL 相互盲法，不继承对方的等级或判据状态。
4. 目标工程案例不参与 ERL 计算，只用于后续独立预测效度验证。
5. 服务故障、预算暂停和调试日志不得改变数据集等级。
6. 正式结果、审计结果、敏感性结果和运维日志必须分开持久化。
